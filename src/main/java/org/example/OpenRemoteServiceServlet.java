...
public class OpenRemoteServiceServlet extends HttpServlet {
	...
	/*
	 * These members are used to get and set the different HttpServletResponse and HttpServletRequest headers.
	 */
	...
	/**
	 * Controls the compression threshold at and below which no compression will take place.
	 */
	private static final int UNCOMPRESSED_BYTE_SIZE_LIMIT = 256;

	/**
	 * Return true if the response object accepts Gzip encoding. This is done by checking that the accept-encoding header specifies gzip as a
	 * supported encoding.
	 */
	...
	/**
	 * This method attempts to estimate the number of bytes that a string will consume when it is sent out as part of an HttpServletResponse. This
	 * really a hack since we are assuming that every character will consume two bytes upon transmission. This is definitely not true since some
	 * characters actually consume more than two bytes and some consume less. This is even less accurate if the string is converted to UTF8. However,
	 * it does save us from converting every string that we plan on sending back to UTF8 just to determine that we should not compress it.
	 */
	...
	public final void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			// result.
			//
			String responsePayload = requestPayload;// processCall(requestPayload);
	...
	/**
	 * Override this method to control what should happen when an exception escapes the {@link #processCall(String)} method. The default
	 * implementation will log the failure and send a generic failure response to the client.
	 * <p/>
	 * <p/>
	 * An "expected failure" is an exception thrown by a service method that is declared in the signature of the service method. These exceptions are
	 * serialized back to the client, and are not passed to this method. This method is called only for exceptions or errors that are not part of the
	 * service method's signature, or that result from SecurityExceptions, SerializationExceptions, or other failures within the RPC framework.
	 * <p/>
	 * <p/>
	 * Note that if the desired behavior is to both send the GENERIC_FAILURE_MSG response AND to rethrow the exception, then this method should first
	 * send the GENERIC_FAILURE_MSG response itself (using getThreadLocalResponse), and then rethrow the exception. Rethrowing the exception will
	 * cause it to escape into the servlet container.
	 * 
	 * @param e
	...
	/**
	 * Gets the <code>HttpServletRequest object for the current call. It
	...
	/**
	 * Gets the <code>HttpServletResponse object for the current call. It
	 * is stored thread-locally so that simultaneous invocations can have
	...
	/**
	 * Override this method to examine the serialized response that will be returned to the client. The default implementation does nothing and need
	...
	/**
	 * Override this method to examine the serialized version of the request payload before it is deserialized into objects. The default
	...
	/**
	 * Determines whether the response to a given servlet request should or should not be GZIP compressed. This method is only called in cases where
	 * the requestor accepts GZIP encoding.
	 * <p/>
	 * This implementation currently returns <code>true if the response
	 ...
	 * @return <code>true if responsePayload should be GZIP compressed,
	...
	/**
	 * Called when the machinery of this class itself has a problem, rather than the invoked third-party method. It writes a simple 500 message back
	 * to the client.
	 */
	private void respondWithFailure(HttpServletResponse response) {
		try {
			...
			getServletContext().log("respondWithFailure failed while sending the previous failure to the client", e);
		}
	}
